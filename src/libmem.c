...
{
  for (struct vm_rg_struct ** p_rgit = &get_vma_by_num(caller->mm, vmaid) ->vm_freerg_list;*p_rgit; p_rgit = &((*p_rgit)->rg_next))
   {
     struct vm_rg_struct *rgit = *p_rgit;
     if (rgit->rg_start + size == rgit->rg_end){
       newrg->rg_start = rgit->rg_start;
       newrg->rg_end = rgit->rg_end;
       *p_rgit = rgit->rg_next; // remove the rgit from the list
       free(rgit);
       return 0;
 
     }
     else if (rgit->rg_start + size < rgit->rg_end)
     {
       newrg->rg_start = rgit->rg_start;
