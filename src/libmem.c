...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  //regs.a3 = ...
  struct sc_regs regs;
  regs.a1 = SYSMEM_INC_OP; /* Operation: increase VMA limit */
  ...
  if (get_free_vmrg_area(caller, vmaid, size, &rgnode) != 0) {
    pthread_mutex_unlock(&mmvm_lock);
    return -1; /* Still failed to allocate */
  }
...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  /* Create a new region node for the freed area */
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    if (find_victim_page(caller->mm, &vicpgn) != 0) {
      return -1; /* Failed to find victim page */
    }
    /* Find victim page */
    ...
    if (MEMPHY_get_freefp(caller->active_mswp, &swpfpn) != 0) {
      return -1; /* No free frame in SWAP */
    }
   ...
   regs.a3 = vicfpn;
    ...
    //mm->pgd[pgn];
    //pte_set_fpn();
    pte_set_swap(&mm->pgd[vicpgn], 0, swpfpn); /* Mark victim as swapped */

    /* Update its online status of the target page */
    pte_set_fpn(&mm->pgd[pgn], vicfpn); /* Mark target as present in RAM */
...
int pg_getval(struct mm_struct *mm, int addr, BYTE *data, struct pcb_t *caller)
{
  ...
  //regs.a3 = ...
  struct sc_regs regs;
  regs.a1 = SYSMEM_IO_READ; /* Operation: read */
...
int pg_setval(struct mm_struct *mm, int addr, BYTE value, struct pcb_t *caller)
{
  ...
  //regs.a3 = ...
  struct sc_regs regs;
  regs.a1 = SYSMEM_IO_WRITE; /* Operation: write */
  regs.a2 = phyaddr;         /* Physical address */
  regs.a3 = value;
  /* SYSCALL 17 sys_memmap */
  syscall(caller, 17, &regs);
  ...
  while (rgit != NULL) {
    ...
    if (region_size >= size) {
      ...
      if (rgit->rg_start == rgit->rg_end) {
        /* Region is fully used, remove it from the list */
        if (prev == NULL) {
